...
# hashable column names for the source data
...
def parse_csv_numeric(csv_value: str) -> float:
    """
    Convert messy input values like " 124,138 " to float. Values of "-" will
...
def parse_safeguard_csv(data_source: ConfiguredDataSource) -> pd.DataFrame:
    """
    Read the Safeguard Mechanism Baselines and Emissions Table CSV, returning
    only the data columns useful for methane estimation.
    """
    return pd.read_csv(
