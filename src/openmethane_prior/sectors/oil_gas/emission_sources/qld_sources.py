...
def qld_emission_sources(
    ...
    boreholes_df = qld_boreholes_df[["borehole_pid", "bore_type", "rig_release_date", "geometry"]]

    # join boreholes with titles to use the title dates as start/end dates
    titles_df = qld_leases_df[["displayname", "approvedate", "expirydate", "geometry"]]
    sources_df = gpd.sjoin(boreholes_df, titles_df, how="inner", predicate="within")
