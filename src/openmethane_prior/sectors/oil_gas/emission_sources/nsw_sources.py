...
def nsw_emission_sources(
    ...
    drillholes_df = nsw_drillholes_df[["geometry", "gsnsw_drill_id", "year_drilled", "business_purpose"]]

    # no specific date, so we'll guess from Jan 1st of the specified year
    drillholes_df["drilled_date"] = drillholes_df["year_drilled"].map(
        lambda year: datetime.datetime(int(year), 1, 1, 0, 0, 0)
    )
    del drillholes_df["year_drilled"]
    ...
    sources_df = gpd.sjoin(drillholes_df, nsw_titles_df, how="inner", predicate="within")
    ...
    # Best guess at an end date for a drillhole is the expiry of the petroleum
    # title. Although this is very inaccurate, we don't have any better public
    # data source that includes the date when wells are capped/expended.
    sources_df = sources_df.rename(columns={"expiry_date": "end_date"})
