...
def test_create_grid_from_domain(input_domain):
    test_grid = create_grid_from_domain(input_domain)

    assert test_grid.dimensions == (input_domain.sizes["x"], input_domain.sizes["y"])
    assert test_grid.shape == (input_domain.sizes["y"], input_domain.sizes["x"])
    ...
    assert test_grid.origin_xy == (input_domain["x_bounds"][0, 0], input_domain["y_bounds"][0, 0])
    assert test_grid.llc_center_xy == (input_domain["x"][0], input_domain["y"][0])
    assert test_grid.cell_area == input_domain.XCELL * input_domain.YCELL


def test_create_grid_from_domain_coordinates(input_domain):
    test_grid = create_grid_from_domain(input_domain)
    ...
    for y in y_test_coords:
        for x in x_test_coords:
            ...
            input_coords = (float(input_domain["lon"][y, x].values), float(input_domain["lat"][y, x].values))
